...
import { User, Phone, X, Mail, CheckCircle, AlertCircle } from "lucide-react"
...
export default function BlogForm(email) {
  ...
  const resetFormCompletely = (resetForm) => {
    ...
    setFieldStates({ name: false, email: false, phoneNumber: false })
    ...
    try {
      ...
      const response = await axios.post(
        "/api/main-form",
        {
          ...
          mailSubject: "New Free Consulation Request Received From Blog Page",
          userEmailSubject: "Thank You for Requesting a Consulation At SevenMentor",
        },
        { timeout: 10000 }
      )
    ...
    <div className="p-4 pb-0 flex items-center justify-center rounded-xl">
      <div className="w-full max-w-md text-center">
        <h3 className="text-white font-bold mb-3 text-2xl md:text-3xl">
          Get Free <span className="animated-text-fill !text-3xl !tracking-tight">Consultation</span>
        </h3>
            ...
            {({ errors, touched, values, isSubmitting }) => (
              <Form className="space-y-4">
                ...
                <div className="lg:pl-[20px] pl-[33px]">
                  <button
                    type="submit"
                    className="cursor-pointer relative w-full mt-2 lg:w-[250px] bg-gradient-to-r from-orange-700 to-orange-500 text-white font-bold py-3 px-4 rounded-full overflow-hidden disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      {showPopup && (
        <div className="fixed inset-0 flex items-center justify-center z-[999999] bg-black/40 backdrop-blur-sm">
          <div className="bg-white p-6 rounded-2xl shadow-2xl w-80 h-80 flex flex-col items-center justify-center text-center relative">
            ...
            >
              <div
                className={`h-10 w-10 flex items-center justify-center ${isSuccess ? "text-green-600" : "text-red-600"}`}
              >
                {isSuccess ? (
            ...
            <p className={`text-center ${isSuccess ? "text-gray-600" : "text-red-500"} px-6 mt-2`}>{popupMessage}</p>
